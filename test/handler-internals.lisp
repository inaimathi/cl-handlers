...
(prove:suite
 (is-expand
  ...
  (LAMBDA ($param-table $header-cb $body-cb)
    (LET (($res-code 200)
      ...
      (FLET ((CL-HANDLERS::REQUEST-HEADER (CL-HANDLERS::NAME)
	       (FUNCALL $header-cb CL-HANDLERS::NAME))
	     (CL-HANDLERS::READ-BODY! ()
	       (FUNCALL $body-cb))
	     (CL-HANDLERS::SET-RESPONSE-CODE! (CL-HANDLERS::CODE)
	       (SETF $res-code CL-HANDLERS::CODE)
	       NIL)
	     (CL-HANDLERS::SET-RESPONSE-HEADER!
		 (CL-HANDLERS::NAME CL-HANDLERS::VALUE)
	...
	(LET ((FOO (STRING-> :INTEGER (FUNCALL $param-table :FOO)))
	  ...
	  (LIST $res-code $res-headers
		(LIST (PROGN (+ FOO BAR BAZ))))))))
 ...
 (is-expand
  ...
  (LAMBDA ($param-table $header-cb $body-cb)
	...
	(LIST $res-code $res-headers (LIST (PROGN "This is a Test"))))))
